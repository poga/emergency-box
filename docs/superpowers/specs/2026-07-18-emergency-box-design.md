...
Three off-the-shelf pieces on the Mac, orchestrated by shell scripts:

- **dnsmasq** — DHCP server (`10.87.0.50–250`) + wildcard DNS resolving
  every hostname to the Mac.
- **Caddy** (port 80) — serves the captive landing page for any host;
  reverse-proxies host `chat.lan` to chatto.
- **chatto** — single Go binary with built-in DB, bound to `127.0.0.1:8080`.
  Open registration, no email verification.
...
│   ├── dnsmasq.conf
│   ├── Caddyfile
│   └── *.plist            # launchd daemons: chatto, dnsmasq, caddy, caffeinate
└── landing/index.html     # self-contained captive landing page, embedded QR
```

## Lifecycle

- **install.sh** (once, online): brew install dnsmasq + caddy; fetch chatto
  darwin-arm64 binary (or build from source with Go — verify during
  implementation); place configs/plists; init chatto data dir with open
  registration; pre-authorize binaries with the application firewall.
  Everything needed later is cached locally.
- **emergency-on**: static IP → `launchctl bootstrap` the four daemons →
...
- chatto started for real; register an account and post a message through
  its actual API — proves offline open registration end-to-end.
- Config validation: `dnsmasq --test`, `caddy validate`.
...
- Chatto darwin-arm64 release artifact vs. build-from-source; exact config
  flags for bind address, base URL, open registration without email.
- Internet Sharing activation from CLI and DHCP coexistence for `--hotspot`.
- Best-effort captive-popup escape UX on iOS (CNA) and Android; landing page
